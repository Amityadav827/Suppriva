-- SUPPRIVA HOMEPAGE TRUST BADGES CMS REPAIR
-- Safe to run multiple times.
-- Creates and hydrates Phase 9 badges when production has not applied
-- the migration yet.
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'homepage_trust_badges_sort_order_check'
      and conrelid = 'public.homepage_trust_badges'::regclass
  ) then
    alter table public.homepage_trust_badges
      add constraint homepage_trust_badges_sort_order_check
      check (sort_order >= 0);
  end if;
end $$;
...
-- Verification:
-- select * from public.homepage_trust_badges order by sort_order;
