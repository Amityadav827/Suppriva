-- SUPPRIVA HOMEPAGE POPULAR PICKS CMS REPAIR
-- Safe to run multiple times.
-- Creates and hydrates Homepage Popular Picks CMS settings when production
-- has not applied the Phase 6 migration yet.
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'homepage_popular_picks_settings_singleton_check'
      and conrelid = 'public.homepage_popular_picks_settings'::regclass
  ) then
    alter table public.homepage_popular_picks_settings
      add constraint homepage_popular_picks_settings_singleton_check
      check (id = 'home');
  end if;
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'homepage_popular_picks_settings_max_products_check'
      and conrelid = 'public.homepage_popular_picks_settings'::regclass
  ) then
    alter table public.homepage_popular_picks_settings
      add constraint homepage_popular_picks_settings_max_products_check check (
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'homepage_popular_picks_settings_sort_mode_check'
      and conrelid = 'public.homepage_popular_picks_settings'::regclass
  ) then
    alter table public.homepage_popular_picks_settings
      add constraint homepage_popular_picks_settings_sort_mode_check check (
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'homepage_popular_picks_settings_source_mode_check'
      and conrelid = 'public.homepage_popular_picks_settings'::regclass
  ) then
    alter table public.homepage_popular_picks_settings
      add constraint homepage_popular_picks_settings_source_mode_check check (
...
-- Verification:
-- select * from public.homepage_popular_picks_settings;
