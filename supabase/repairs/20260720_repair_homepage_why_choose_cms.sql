-- SUPPRIVA HOMEPAGE WHY CHOOSE CMS REPAIR
-- Safe to run multiple times.
-- Creates and hydrates Phase 8 cards when production has not applied
-- the migration yet.
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'homepage_why_choose_cards_sort_order_check'
      and conrelid = 'public.homepage_why_choose_cards'::regclass
  ) then
    alter table public.homepage_why_choose_cards
      add constraint homepage_why_choose_cards_sort_order_check
      check (sort_order >= 0);
  end if;
end $$;
...
-- Verification:
-- select * from public.homepage_why_choose_cards order by sort_order;
